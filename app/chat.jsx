...
export default function ChatScreen() {
  ...
  const handleGenerateFruit = () => {
    ...
    try {
      ...
      // 🏆 2. 關鍵改動：不再彈出醜 alert，而是把水果塞給狀態，並把精美 Modal 打開！
  ...
  // 🎯 處理點擊「🍊 生成果實」按鈕的邏輯（徹底移除 setTimeout 穩定版）
  // const handleGenerateFruit = () => {
  //   console.log("=== 🚨 偵錯點 1：按鈕成功被點擊了！ ===");
  ...
  //   try {
  //     // 🎲 1. 核心功能：立刻從本機素材庫「隨機抽出一顆果實」
  //     const fruit = getRandomFruit(); 
  //     console.log("=== 🚨 偵錯點 3：成功抽到的水果是 ===", fruit.name);

  //     // 🏆 2. 網頁版測試保險：使用網頁通用的原生 alert，確保一定彈得出來
  //     alert(`✨ 凝聚成功！\n今日的心情成功提煉出了【${fruit.name}】！已自動放入故事背包。`);
      
  //     console.log("🚀 準備帶著參數跳轉至怪獸頁面...", fruit.iconKey);

  //     // 🚀 3. 核心快遞：不要等 1 秒！立刻把水果資料帶到怪獸頁面
  //     router.push({
  //       pathname: '/monster',
  //       params: {
  //         newFruitKey: fruit.iconKey,
  //         newFruitName: fruit.name,
  //         newFruitExp: fruit.bonus_exp,
  //         newFruitText: fruit.effect_text,
  //       }
  //     });

  //   } catch (error) {
  //     console.error("=== 🚨 前端抽卡炸爐了 ===", error);
  //     alert("果實凝聚失敗，請查看開發者主控台。");
  //   }
  // };
  // const handleGenerateFruit = async () => {
  ...
  //   console.log("🍊 準備傳送今日心情至果實煉金爐...", userConversations);
  //   setIsAiLoading(true);

  //   setTimeout(() => {
  //     try {
  //       // 🎲 2. 核心功能：直接從你的本機素材庫「隨機抽出一顆果實」！
  //       const fruit = getRandomFruit(); 
  ...
  //       Alert.alert(
  //         "✨ 凝聚成功", 
  //         `今日的心情成功提煉出了【${fruit.name}】！已自動放入你的故事背包。`,
  //         [
  //           { 
  //             text: "前往餵食魔獸", 
  //             onPress: () => {
  //               // 🚀 3. 打包這顆隨機抽到的水果資料，絲滑跳轉送到怪獸頁面！
  //               router.push({
  //                 pathname: '/monster',
  //                 params: {
  //                   newFruitKey: fruit.iconKey,
  ...
  //       );
  //     } catch (error) {
  //       console.log("=== 🚨 煉金術炸爐了，錯誤訊息是 ===", e);
  //       console.error(error);
  //       setIsAiLoading(false);
  //     }
  //   }, 1000); // 讓它轉圈圈 1 秒鐘，比較有煉金術凝聚的儀式感！
  // };
      ...
      >
        ...
        <View style={styles.header}>
          <View style={styles.headerLeft}>
            ...
            <Text style={[styles.headerTitle, { fontSize: 15 }]}>{currentDateText}</Text>
          </View>
            ...
            <View style={{ flexDirection: 'row', width: '100%', gap: 10, justifyContent: 'center', marginTop: 10 }}>
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  //alert(`🎒 【${rewardFruit?.name}】已成功放入故事背包！`);
                }}
                style={{
                  flex: 1,
